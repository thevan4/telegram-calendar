...
func (k KeyboardFormer) GenerateCalendarKeyboard(
	...
	case selectDayAction:
		return InlineKeyboardMarkup{}, formDateTime(incomePayload.calendarDay, incomePayload.calendarMonth,
...
func (k KeyboardFormer) getMonthsButtons(month, year int, needShowSelectedMonth bool) (
	...
	btnPrevMonth = NewInlineKeyboardButton(prevMonthActionName, k.Encoding(prevMonthAction, zero, month, year))
	btnNextMonth = NewInlineKeyboardButton(nextMonthActionName, k.Encoding(nextMonthAction, zero, month, year))

	// To be able to return to the current month by pressing again.
	if needShowSelectedMonth {
		btnMonth = NewInlineKeyboardButton(k.monthNames[month-1], k.Encoding(showSelectedAction, zero, month, year))
	} else {
		btnMonth = NewInlineKeyboardButton(k.monthNames[month-1], k.Encoding(selectMonthAction, zero, month, year))
	}
...
func (k KeyboardFormer) getYearsButtons(month, year int, needShowSelectedYear bool) (
	...
	btnPrevYear = NewInlineKeyboardButton(prevYearActionName, k.Encoding(prevYearAction, zero, month, year))
	btnNextYear = NewInlineKeyboardButton(nextYearActionName, k.Encoding(nextYearAction, zero, month, year))

	// To be able to return to the current year by pressing again.
	if needShowSelectedYear {
		btnYear = NewInlineKeyboardButton(strconv.Itoa(year), k.Encoding(showSelectedAction, zero, month, year))
	} else {
		btnYear = NewInlineKeyboardButton(strconv.Itoa(year), k.Encoding(selectYearAction, zero, month, year))
	}
...
func (k KeyboardFormer) formBtnBeauty(month, year int, currentUserTime time.Time) InlineKeyboardButton {
	...
	return NewInlineKeyboardButton(k.homeButtonForBeauty, k.Encoding(beautyCallback, zero, curMonth, curYear))
}
...
func (k KeyboardFormer) addDaysNamesRow(curMonth, curYear int) (rowDays []InlineKeyboardButton) {
	...
	for _, day := range k.daysNames {
		btn := NewInlineKeyboardButton(day, k.Encoding(silentDoNothingAction, zero, curMonth, curYear))
...
func (k KeyboardFormer) addMonthsNamesRow(year int) (rowMonthsOne, rowMonthsTwo []InlineKeyboardButton) {
	...
	for month := 1; month <= 6; month++ {
		btn := NewInlineKeyboardButton(k.monthNames[month-1], k.Encoding(showSelectedAction, zero, month, year))
...
func (k KeyboardFormer) addYearsNamesRow(month, currentYear int) (rowYears []InlineKeyboardButton) {
	...
	for year := currentYear - k.yearsBackForChoose; year < currentYear; year++ {
		btn := NewInlineKeyboardButton(strconv.Itoa(year), k.Encoding(showSelectedAction, zero, month, year))
		rowYears = append(rowYears, btn)
	}

	// Current year.
	btnCur := NewInlineKeyboardButton(strconv.Itoa(currentYear), k.Encoding(showSelectedAction, zero, month, currentYear))
